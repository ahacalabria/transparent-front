...
class CadastrarColaborador extends Component {
    ...
    constructor(props){
      ...
      async getGrupos(){    
        try {
          await handleRequest('grupo/colaboradores', {}, 'GET', 'grupos', this)  
        } catch(err) {
          this.props.alert.error('Error')
        }
      }
      ...
      async saveUser(e, dados) {
        e.preventDefault()
            ...
            const url_tratada = (method_action==="PUT") ? 'users/create_colaborador/'+this.state.id : 'users/create_colaborador'
    ...
    if(this.state.grupos)
      gruposSelect = this.state.grupos.map((e, key) => {return <option key={key} value={e.id}>{e.nome_grupo}</option>})
    ...
    if(this.state.estado_id!=='' && this.state.estado_id===this.state.estado_id_previous){
      ...
      //   if(this.state.instituicoes) instituicoesOptions = this.state.instituicoes.map( (e, key) => {return <option key={key} value={e.id}>{e.razao_social}</option>})
        ...
        <h1>Cadastro de  <small>OPERADOR</small></h1>
      </div>
            ...
            <FormGroup controlId="cnpj" bsSize="large">
              <ControlLabel><Required/> Grupos</ControlLabel>
              <FormControl componentClass="select" name="grupo_id" value={this.state.grupo_id} onChange={this.onChange.bind(this)} required>
...
export default withAlert(CadastrarColaborador)
